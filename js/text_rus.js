...
var texttraits = {
	TRAIT_FAST_METABOLISM: ['Быстрый метаболизм', 'Ваш метаболизм превышает норму. Вам дается +15 к уровню лечения, +8 к силе исцеления суперстимпаков. Устойчивость к яду и радиации в 2 раза меньше. Любое изменение ОД добавит вам 1 жизнь, но только 1 раз за действие'],
	...
	TRAIT_HEAVY_HANDED: ['Вор', ' +15 к Уровню лечения. Холодное одноручное оружие и метательные ножи наносят +10 конечного урона, но точность другого оружия равна 5%. Бонусы для вора. Вы не можете наносить критические удары. Нельзя взять Бесшумный бег.'],
	...
	TRAIT_GOOD_NATURED: ['Добродушие', 'Боевые навыки меньше, а мирные-больше. Вы берете перки каждые 2 уровня, но не можете взять уберперки и Бесшумный бег. Ваш уворот делится пополам. Большинство гуманоидов вас не атакуют, но звери атакуют без задержки.'],
	TRAIT_CHEM_RELIANT: ['Химик', 'Длительность наркотиков увеличено в 3 раза. Откаты длятся 30 секунд. Шанс получить зависимость увеличен в 2 раза. Зависимость длятся в 3 раза дольше.'],
	...
	TRAIT_SKILLED: ['Умелец', 'Вы получаете +25 к антикриту, +15 к шансу крита, +4 параметра к статам, 4-й тагнутый навык и +3 к скиллпоинтам за каждый уровень. Но перки начисляются только после каждого 4 уровня, зато боевой режим при вашей атаке длится 5 секунд. Не можете взять Уберперки и Бесшумный бег.'],
...
var textperk = {
	...
	PE_ACTION_BOY: ['Человек действия', 'Человек действия все операции совершает бегом, наподобие новобранца. Это придает ему дополнительное Очко Действия, которое можно использовать в любых целях.'],
	...
	PE_BIVALIY: ['Крепкий Орешек', 'Вы можете ослабить критический урон и эффект по вам. -((ВС*3)+СЛ) к Критроллу. Резка критурона зависит от Силы, Удачи и Интеллекта. Режет критурон Нокаута и Пропуска хода с 75% до 50% и с 50% до 30% соответственно.'],
	// NEW PERK >
	...
	PE_BIGSKIN: ["Горячая кровь", "Иммунитет к контузии. Вы не можете критически промахнуться."],
	...
	PE_TRAPPER: ["Цепной пёс", "Пули вас так просто не остановят. Если до противника меньше 16 гексов, то вы получаете (15-дистанция)% к увороту против его дистанционных атак, но не более 45%"],
	// NEW PERK >>
	...
	PE_HTH_EVADE: ['Верткость', 'Вы чувствуете, когда надо дернуться с места, природная ловкость помогает вам в этом. Процент Уклонения от атаки увеличивается на: (ТекущОД/2).'],
	...
	PE_VAULT_CITY_INOCULATIONS: ['Кабан', "Вы всегда идёте к цели напролом. Если на дистанции менее 15 гексов от цели по вам проходит более одного выстрела в секунду, урон по вам срезается на 10%."],
	...
	// NEW PERK >>
	PE_KEEN: ["Зоркий", " Режется штраф одиночного выстрела в дым (60-дистанция)/2. В случае выстрела очередью дым игнорируется.Вшит функционал перка прицел"],
