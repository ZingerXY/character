...
var texttraits = {
	TRAIT_FAST_METABOLISM: ['Быстрый метаболизм', 'Ваш метаболизм превышает норму. Вам дается +(Харизма*3) к уровню лечения, +12 к силе исцеления суперстимпаков. Устойчивость к яду и радиации в 2 раза меньше. Любое изменение ОД добавит вам 1 жизнь, но только 1 раз за действие'],
	...
	TRAIT_KAMIKAZE: ['Камикадзе', 'Вы прете напролом. +1 Ловкость. Эффект "Подавление" обнуляется первым тиком. На вас не действует защита в дыму.'],
	TRAIT_HEAVY_HANDED: ['Вор', 'Холодное одноручное оружие и метательные ножи наносят +10 конечного урона, но точность другого оружия равна 5%. Бонусы для вора. Вы не можете наносить критические удары.'],
	...
	TRAIT_JINXED: ['Дурной глаз', 'Хорошая сторона дела заключается в том, что ваши враги часто ошибаются. Плохо то, что вы ошибаетесь не меньше их! Повышенный шанс критического промаха у вас и ваших жертв. В промахи не входят увороты.'],
	TRAIT_GOOD_NATURED: ['Добродушие', 'Боевые навыки меньше, а мирные-больше. Вы берете перки каждые 2 уровня, но не можете взять уберперки. Ваш уворот равен нулю. Большинство гуманоидов вас не атакуют, но звери атакуют без задержки. Реген очков жизни всегда половина от Уровня лечения.'],
	...
	TRAIT_SEX_APPEAL: ['Жидкое тело', 'Вы получаете -15% на сопротивление очередям и типу урона Огонь, Максимальнsq вес +50 кг. Иммунитет к тикам огня. Лечащие препараты не действуют в полную мощь (-25% при лечении).'],
	TRAIT_SKILLED: ['Умелец', 'Вы получаете +4 параметра к статам, 4-й тагнутый навык и +3 к скиллпоинтам за каждый уровень. Но перки начисляются только после каждого 4 уровня, зато боевой режим при вашей атаке длится 5 секунд.'],
...
var textperk = {
	...
	PE_SILENT_DEATH: ['Хитрость', 'Вы хитры в своих действиях. +30 ОЖ, +5% к Увороту, +50% к Атлетизму. Уходя в сник, вы выходите из боевого режима, при условии, что вы одеты в кожанку (боевая).'],
	...
	PE_HEAVE_HO: ['Счастливчик', 'При ролле крита по вам добавляется Удача. Попадание в вас с шансом (Удача+4-Удача противника)% не нанесет урона. Доктором можно восстановить все ОД и ОЖ (шанс Удача%).'],
	...
	PE_FLOWER_CHILD: ['Полевой санитар', 'Восстановление навыка Санитар меньше на 4 секунды. При лечении санитаром добавляются (ИН+УД)*3 дополнительные ОЖ, а также можно восстановить все ОЖ с шансом (Удача*4). Игнор эффекта Вялость.'],
	...
	PE_BOOKWORM: ['Регенерация', 'Пассивное восстановление ОЖ в режиме боя. Лечит конечности во время тика раз в 10 секунд, если персонаж не в бою.'],
	...
	PE_BIGSKIN:["Толстокожий","От типов урона Огонь, Лазер, Электро вы получаете на 15% меньше повреждений."],
	...
	PE_FISTRAGE:["Критическое мышление","Каждый выстрел увеличивает на 10% критшанс. После выпадении крита он обнуляется."],
	PE_ADEPTGLOW: ["Рикошет", "Если в эффекте Цель по вам пройдет больше чем 1 выстрел, то второй и последующие пройдут по касательной с рандомным (1, урон) уроном."],
	PE_FANAT: ["Разгрузка", "У вас встроенная разгрузка. Вы можете использовать предметы затрачивая на 1 Очко действия меньше."],
	PE_STOIK: ['Стоик', 'Ваш реген Очков Действия становится быстрее. 1 уровень-на 10%, 2 уровень-20%, 3 уровень-30%'],
	PE_CRITTER: ["Концентрация", "Вы получаете -1 очко действия на прицельные выстрелы. Кроме инженерного оружия."],
	PE_TARGETING: ['Лидер', 'При атаке игрока одиночным выстрелом/ударом, который с вами в одной фракции, тот игрок восстанавливает все од, вместо урона. Не действует на Арройо и не работает в эффекте Цель. Таймауты Доктора должны быть равны нулю. Включает таймауты Доктора на 60 секунд у обоих.'],
	...
	PE_TRAPPER: ["Сапер","Изучение капканов и взрывных устройст дало вам стойкость к норме, плазме и взрыву (срез урона на 15% к Норме, Плазме и Взрыву)."],
	// NEW PERK >>
	...
	PE_ARMOR: ['Броня', 'Ваше тело как броня. Вам добавлено 75 очков жизни.'],
	// << NEW PERK
	...
	// << NEW PERK
	PE_JINXED_II: ['Дурной глаз', 'Хорошая сторона дела заключается в том, что враги вокруг вас часто ошибаются. Плохо то, что вы ошибаетесь не меньше их!'],
	...
	PE_NAPROLOM: ['Бывалый', 'Вы знаток оружия. +40% к навыку Легкое, Тяжелое, Энергетическое оружие.'],
	PE_ADW_MET: ["Карман", "У вас карман стал глубже и можно добавить 1 веса."],
	// NEW PERK >>
	PE_FISTSANITAR: ['Военврач', '-15 секунд к восстановлению Санитара. Лечение Санитаром как при 10 Удачи. С шансом (11-ХаризмаПротивника) вешает эффект Вялость при атаке (только одиночным выстрелом или ударом).'],
	...
	PE_FIRESHOOT: ['Точный расчет', 'Вы много тренировались и режете уворот цели пополам.'],
	// << NEW PERK
	...
	PE_PHOENIX_IMPLANTS: ['Реаниматор', 'Вы отличный врач. Любое изменение очков жизни (неважно, плюс или минус) добавит к ним (навык Доктор/55) единицы жизней.'],
	...
	PE_OFFICER: ['Офицер', 'Вы умеете управлять большим отрядом бойцов. Вы получаете +30 ОЖ, а также точность +5%(так же 5% к точности получают и те, кто к вам присоединен). Используется выносливость при расчете группы. Подавление от оружия делится пополам.'],
