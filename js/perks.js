...
var perk = {	//(перк, уровней перка, мин уровень взятия, максимальный уровень взятия, [требования])
	...
	PE_TOUGHNESS: ["Крутизна", "Крутизна позволяет чувствовать себя под защитой. За каждый уровень этой способности вы получаете +7% к Резисту от нормы, огня, взрыва и Сопротивляемость урона от нормы на 1 единицу.",
		...
		function(){pr.addr("normal",1,7);pr.addr("fire",0,7);pr.addr("explode",0,7);},
		function(){pr.addr("normal",-1,-7);pr.addr("fire",0,-7);pr.addr("explode",0,-7);},
	...
	PE_ATTACK: ["Неудержимый", "Вы неудержимы. Получаете +очков действия и +процент к урону согласно вашей Ловкости. Нельзя взять Снайпера, Терма и Человека Действия.",
		...
		function(){pr.add("feats","apoi",Math.floor(stats.AGI[2]/2));}, 
		function(){pr.add("feats","apoi",-Math.floor(stats.AGI[2]/2));},
    ...
    PE_PRO_UDAR: ["Весельчак", "Вы не падаете при употреблении алкоголя, а наоборот получаете бонус к защите -10%.",
		1,6,29,function(n){return stats.STR[2] >= 10 && (n||skills.melee[0] >= 100 && skills.steel[0] >= 100)},
		function(){},
		function(){},
