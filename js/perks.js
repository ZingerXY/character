...
var perk = {	//(перк, уровней перка, мин уровень взятия, максимальный уровень взятия, [требования])
	...
	PE_FASTER_HEALING: ["Быстрое лечение", "Вы получаете +15 к Уровню лечения, таким образом, ваши раны быстрее заживают. Восстановление 1\4 максимальных ОД при исцелении Санитаром. Использование препаратов на 100мс быстрей.",
		1,6,29,function(n){return stats.ENU[2] >= 8;},
	...
	PE_TOUGHNESS: ["Крутизна", "Крутизна позволяет чувствовать себя под защитой. Вы получаете +2 к трешхолду от Нормы  Лазера, +10% к Резисту от Огня и Взрыва.",
		1,2,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.ENU[2] >= 8 && stats.STR[2] >= 6;},
		function(){pr.addr("normal",2,0);pr.addr("laser",2,0);pr.addr("fire",0,10);pr.addr("explode",0,10);},
		function(){pr.addr("normal",-2,0);pr.addr("laser",-2,0);pr.addr("fire",0,-10);pr.addr("explode",0,-10);},
		{stats:{ENU:8,STR:6},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_SILENT_RUNNING: ["Бесшумный бег", "Теперь вы будете двигаться быстро, не издавая больше шума. Ранее это было совершенно неосуществимо в силу технических причин. Несовместим с перком - Снайпер.",
		1,12,29,function(n){return !checkperk("PE_CRITTER") && !checkperk("PE_COWBOY") && !checkperk("PE_ATTACK") && !checkperk("PE_SNIPER") && !("TRAIT_SKILLED" in mychar.traits) && !("TRAIT_HEAVY_HANDED" in mychar.traits);},
		function(){},
		function(){},
		{perks:{PE_SNIPER:1,PE_ATTACK:1,PE_COWBOY:1,PE_CRITTER:1},traits:{TRAIT_SKILLED:1,TRAIT_HEAVY_HANDED:1}}],
	...
	PE_FORTUNE_FINDER: ["Воодушевление", "Каждый Уворот от вражеской атаки прибавляет вам +1 ОД. Увеличивает Уворот на Ловкость%. Если ОЖ меньше трети от максимума, вы не получаете криты.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits);},
		function(){pr.add("feats","dodge",stats.AGI[2]);},
		function(){pr.add("feats","dodge",stats.AGI[2]);},
		{traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_SNIPER: ["Снайпер", "Шанс нанести Крит увеличивается на 30%. -1 ОД на прицельный выстрел. При одиночном прицельном выстреле может выбить Байпасс брони.",
		1,15,29,function(n){return !checkperk("PE_CRITTER") && !checkperk("PE_ATTACK") && !checkperk("PE_SILENT_RUNNING") && !checkperk("PE_TERMINATOR") && !checkperk("PE_DEFENCE") && !("TRAIT_GOOD_NATURED" in mychar.traits);},
		function(){pr.add("feats","crit",30);},
		function(){pr.add("feats","crit",-30);},
		{perks:{PE_ATTACK:1,PE_SILENT_RUNNING:1,PE_TERMINATOR:1,PE_DEFENCE:1,PE_CRITTER:1},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_ACTION_BOY: ["Человек действия", "Человек действия все операции совершает бегом, наподобие новобранца. Это придает ему дополнительных 2 Очка Действия, которые можно использовать в любых целях.",
		1,15,29,function(n){return !checkperk("PE_ANIMAL_FRIEND") && !checkperk("PE_QUICK_RECOVERY") && stats.AGI[2] >= 6;},
		function(){pr.add("feats","apoi",2);},
		function(){pr.add("feats","apoi",-2);},
		{stats:{AGI:6},perks:{PE_QUICK_RECOVERY:1,PE_ANIMAL_FRIEND:1}}],
	PE_MENTAL_BLOCK: ["Стойкость", "Вы ну прямо очень сильны и устойчивы. Вы получаете Сила*2 ОЖ, +5% к Резисту Нормы и Электро, -2 к тикам плазмы. Нагрев накапливается в 2 раза медленней.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !("TRAIT_SEX_APPEAL" in mychar.traits) && !checkperk("PE_BIGSKIN") && stats.STR[2] >= 8 && stats.INT[2] >= 8},
		function(){pr.addr("electro",0,5);pr.addr("normal",0,5);pr.add("feats","live",stats.STR[2]*5);},
		function(){pr.addr("electro",0,-5);pr.addr("normal",0,-5);pr.add("feats","live",-stats.STR[2]*5);},
		{stats:{STR:8,INT:8},perks:{PE_BIGSKIN:1},traits:{TRAIT_GOOD_NATURED:1,TRAIT_SEX_APPEAL:1}}],
	...
	PE_CULT_OF_PERSONALITY: ["Отличник", "Вы отлично учились и многое усвоили. -6 Урона по вам от типов урона Плазма, Электричество. Дает иммунитет к сжиганию ОД.",
		1,2,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.PER[2] >= 7 && stats.INT[2] >= 8},
		function(){},
		function(){},
		{stats:{INT:8,PER:7},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_TAG: ["Математик", "Вы все досконально просчитываете. В бою вы получаете +30 к используемому навыку и уменьшаете штраф от дыма на 15, а укрытий на 10. В то же время получая +10 к укрытиям и +15 к дыму когда вы атакованы.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.INT[2] >= 7 && stats.LUC[2] >= 2 && (n||skills.science[0] >= 200);},
		function(){},
		function(){},
		{stats:{INT:7,LUC:2},skills:{science:200},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_PALESKIN:["Бледнокожий","Вы не очень любите дневной свет и пристальные взгляды. А вот стимуляторы вам по душе. И вы их не колете.. вы их пьете. Сски отхиливают +6хп. Ночью с 20:00 до 08:00 у ссок нет откатов. Исключает Реаниматора.",
		1,2,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !checkperk("PE_PHOENIX_IMPLANTS");},
		function(){},
		function(){},
		{traits:{TRAIT_GOOD_NATURED:1},perks:{PE_PHOENIX_IMPLANTS:1}}],
	PE_GECKON:["Ящерица","У вас была терминальная стадия рака и единственным выходом было нетрадиционное лечение. Никто не знал, что инъекции стволовых клеток ящерицы обернутся таким приятным сюрпризом. Входящий по вам урон свыше 65 ед. с вероятностью 50% восстановит Сила*2 ОЖ.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.STR[2] >= 7;},
		function(){},
		function(){},
		{stats:{STR:7},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_GUY:["Крутой парень","Игнорирование требования Силы на дальность метания. Ваша атака по сопернику и атака соперника по вам имеет шанс (Восприятие/2+Сила/2)% сжечь ему 3 Од. Сжигание ОД работает на погексовые атаки.",
		1,6,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && (n||skills.thrown[0] >= 120);},
		function(){pr.add("feats","live",20);},
		function(){pr.add("feats","live",-20);},
		{skills:{thrown:120},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_BIGSKIN:["Толстокожий","От типов урона норма, взрыв, импульс, плазма вы полчаете на 15% меньше повреждений.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !checkperk("PE_MENTAL_BLOCK") && stats.STR[2] >= 8 && (n||skills.orderly[0] >= 220);},
		function(){},
		function(){},
		{skills:{orderl:220},perks:{PE_MENTAL_BLOCK:1},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_MASTERMELE:["Мастер ближнего боя","Шанс (Восприятие+Харизма)% заблокировать или отразить в атакующего 2/3 урона, нанесенного с расстояния до 1-6 гексов.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits);},
		function(){},
		function(){},
		{traits:{TRAIT_GOOD_NATURED:1}}],
	PE_COWBOY:["Ковбой","Все бонусы действуют только для огнестрельных пистолетов. Прицельные атаки вернут 1 од (если нет перка Снайпер). Шанс крита +5%. Если не в бою, то выстрел обнуляет патроны цели  в магазине. Исключает Бесшумный бег.",
		1,6,29,function(n){return !checkperk("PE_SILENT_RUNNING") && stats.AGI[2] >= 8 && (n||skills.light[0] >= 180);},
		function(){pr.add("feats","crit",5);},
		function(){pr.add("feats","crit",-5);},
		{skills:{light:180},stats:{AGI:8},perks:{PE_SILENT_RUNNING:1}}],
	PE_FISTRAGE:["Кулак ярости","Ваши яростые атаки выбивают из колеи любого противника, позволяя нанести оглушительный удар. Каждая пятая атака по цели накладывает эффект контузия, при учете того, что она наносит минимум 60 урона.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !("TRAIT_HEAVY_HANDED" in mychar.traits) && (n||skills.melee[0] >= 170);},
		function(){},
		function(){},
		{skills:{melee:170},traits:{TRAIT_GOOD_NATURED:1,TRAIT_HEAVY_HANDED:1}}],
	PE_ADEPTGLOW:["Адепт Глоу","Иммунитет к облучению. Поглощение из урановой руды 250 пунктов радиации. Поголощение происходит путем использования доктора на обогащенный уран в инвентаре. Действует 2 минуты и откаты 6 минут. Поглощенные пункты тратятся вместе с атаками и при поглощении урона.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.ENU[2] >= 8 && stats.CHA[2] >= 4;},
		function(){},
		function(){},
		{stats:{ENU:8,CHA:4},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_FANAT:["Фанатик","Вы остаетесь в сознании даже получая смертельные раны. При получении такого ранения ваше здоровье останавливается на значении 1хп. По истечении 3 секунд, если ваше здоровье меньше 50% от мах, вы умираете. Кд способности 1 минута. Ислючает Стоика.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !checkperk("PE_STOIK") && stats.STR[2] >= 8 && stats.LUC[2] >= 8;},
		function(){},
		function(){},
		{stats:{STR:8,LUC:8},perks:{PE_STOIK:1},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_STOIK:["Стоик","Хабологи хранят в строжайшей тайне то, что заставляет вас возвращаться к жизни вновь и вновь. При получении летального урона персонаж не умрет и восстановит здоровье по формуле (Уд+Хар)2+Вн3=%. Кд способности минута. Исключает Фаната.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !checkperk("PE_FANAT") && stats.INT[2] >= 8 && stats.ENU[2] >= 10;},
		function(){},
		function(){},
		{stats:{INT:8,ENU:10},perks:{PE_FANAT:1},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_CRITTER:["Существо","Вражеские атаки вас только злят. И чем вы злее, тем опаснее. При здоровье <50% +Харизма% урона. При убийстве игрока восстанавливает Харизма*10% здоровья и (Харизма+удача)/2 Од. Уберперк.",
		1,15,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && !checkperk("PE_SILENT_RUNNING") && !checkperk("PE_ATTACK") && !checkperk("PE_SNIPER") && !checkperk("PE_TERMINATOR");},
		function(){},
		function(){},
		{perks:{PE_SNIPER:1,PE_TERMINATOR:1,PE_SILENT_RUNNING:1,PE_ATTACK:1},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_TARGETING:["Пристрелка","При атаке одной и той же цели вы получаете +2 урона и +3 точности за каждое попадание вплоть до +10 и +15 соответственно. При смене цели бонусы пропадают.",
		1,12,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.AGI[2] >= 8 && stats.LUC[2] >= 2;},
		function(){},
		function(){},
		{stats:{AGI:8,LUC:2},traits:{TRAIT_GOOD_NATURED:1}}],
	// NEW PERK
	...
	PE_ATTACK: ["Неудержимый", "Вы неудержимы как... Да вас хрен остановишь! Вы получаете +ЛВ/2 ОД. и (ЛВ + ВС)*5 процент к навыку Атлетизм. Нельзя взять Снайпера, Терминатора и Человека Действия.",
		1,15,29,function(n){return !checkperk("PE_CRITTER") && !checkperk("PE_SILENT_RUNNING") && !checkperk("PE_SNIPER") && !checkperk("PE_TERMINATOR") && stats.AGI[2] >= 2;},
		function(){pr.add("feats","apoi",Math.floor(stats.AGI[2]/2-1));},
		function(){pr.add("feats","apoi",-Math.floor(stats.AGI[2]/2-1));},
		{stats:{AGI:2},perks:{PE_SNIPER:1,PE_TERMINATOR:1,PE_SILENT_RUNNING:1,PE_CRITTER:1}}],
	...
	PE_TERMINATOR: ["Терминатор", "Все Критические эффекты противника игнорируются в зависимости от вашей Выносливости*5% и природного антикрита.",
		1,15,29,function(n){return !checkperk("PE_SNIPER") && !checkperk("PE_ATTACK");},
		function(){},
		function(){},
		{perks:{PE_SNIPER:1,PE_ATTACK:1}}],
	PE_IRON_MAN: ["Взрывотехник", "С этой способностью вы можете разминировать территорию в радиусе 1 гекс(Наука на себя), переключатели пластида действуют быстрей и есть возможность делать роботов-шахидов (детали робота+пластид).",
		1,12,29,function(n){return stats.LUC[2] >= 5 && (n||skills.science[0] >= 150 && skills.traps[0] >= 200);},
		function(){},
		function(){},
	...
	PE_PRO_UDAR: ["Токсиколог", "Вы достигли небывалых высот в химии. Любое ваше оружие наносит дополнительный урон отравлением.",
		1,9,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && (n||skills.doctor[0] >= 200);},
		function(){},
		function(){},
		{skills:{doctor:200},traits:{TRAIT_GOOD_NATURED:1}}],
	PE_ADD_ATAC: ["Дополнительные атаки", "Отдача от оружия никогда не была для вас проблемой. При выстреле с вероятностью (ОД на выстрел * 10)% есть шанс восстановить 2 Очка Действия.",
		1,12,29,function(n){return stats.AGI[2] >= 7 && stats.INT[2] >= 7},
		function(){},
		function(){},
		{stats:{AGI:7,INT:7}}],
	...
	PE_PHOENIX_IMPLANTS: ["Реаниматор", "Вы отличный врач. Любое изменение очков жизни (неважно, плюс или минус) добавит к ним 4 единицы, при условии, что вы не отравлены.",
		1,12,29,function(n){return (n||skills.doctor[0] >= 200) && !checkperk("PE_PALESKIN");},
		function(){},
		function(){},
		{skills:{doctor:200},perks:{PE_PALESKIN:1}}],
	...
	PE_SUPPORTER: ["Огневая поддержка", "Стрельба очередями \"Подавляет\" врага. Увеличен радиус Дымовой и Газовой завесы, и область поджигания Кокт. \"Молотова\". Также вы получате +20% к навыку Метательное.",
		1,12,29,function(n){return (n||skills.thrown[0] >= 100) && stats.PER[2] >= 7 && stats.AGI[2] >= 7 && !("TRAIT_GOOD_NATURED" in mychar.traits);},
		function(){pr.add("skills","thrown",20,1);},
		function(){pr.add("skills","thrown",-20,1);},
		{stats:{PER:7,AGI:7},skills:{thrown:100},traits:{TRAIT_GOOD_NATURED:1}}]
}
