...
var perk = {	//(перк, уровней перка, мин уровень взятия, максимальный уровень взятия, [требования])
	...
	PE_SNIPER: ["Снайпер", "Шанс нанести Крит увеличивается на 30%. -1 ОД на прицельный выстрел. При одиночном прицельном выстреле может выбить Байпасс брони.",
		1,15,29,function(n){return !checkperk("PE_ATTACK") && !checkperk("PE_SILENT_RUNNING") && !checkperk("PE_TERMINATOR") && !checkperk("PE_DEFENCE") && !("TRAIT_GOOD_NATURED" in mychar.traits);},
		function(){pr.add("feats","crit",30);},
		function(){pr.add("feats","crit",-30);},
		{perks:{PE_ATTACK:1,PE_SILENT_RUNNING:1,PE_TERMINATOR:1,PE_DEFENCE:1},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_TAG: ["Математик", "Вы все досконально просчитываете. В бою вы получаете +30 к используемому навыку и уменьшаете штраф от дыма на 15, а укрытий на 10. В то же время получая +10 к укрытиям и +15 к дыму когда вы атакованы.",
		1,6,29,function(n){return !("TRAIT_GOOD_NATURED" in mychar.traits) && stats.INT[2] >= 8 && (n||skills.science[0] >= 180);},
		function(){},
		function(){},
		{stats:{INT:8},skills:{science:180},traits:{TRAIT_GOOD_NATURED:1}}],
	...
	PE_ATTACK: ["Неудержимый", "Вы неудержимы как... Да вас хрен остановишь! Вы получаете +ЛВ/2 ОД. и (ЛВ + ВС)*5 процент к навыку Атлетизм. Нельзя взять Снайпера, Терминатора и Человека Действия.",
		1,15,29,function(n){return !checkperk("PE_SILENT_RUNNING") && !checkperk("PE_ACTION_BOY") && !checkperk("PE_SNIPER") && !checkperk("PE_TERMINATOR");},
		function(){pr.add("feats","apoi",Math.floor(stats.AGI[2]/2));pr.add("skills","speed",(stats.PER[2]+stats.AGI[2])*5,1);},
		function(){pr.add("feats","apoi",-Math.floor(stats.AGI[2]/2));pr.add("skills","speed",-(stats.PER[2]+stats.AGI[2])*5,1);},
		{perks:{PE_ACTION_BOY:1,PE_SNIPER:1,PE_TERMINATOR:1,PE_SILENT_RUNNING:1}}],
