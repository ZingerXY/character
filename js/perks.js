var perk = {	//(перк, уровней перка, мин уровень взятия, максимальный уровень взятия, [требования])
	...
	PE_BONUS_HTH_DAMAGE: [0,3,30,99,function(n){return !traits.traits10[1] && stats.agi[2] >= 6 && stats.str[2] >= 6;}],
	PE_BONUS_MOVE: [0,2,3,29,function(n){return stats.agi[2] >= 5;}],
	PE_BONUS_RANGED_DAMAGE: [0,1,3,29,function(n){return !traits.traits10[1] && stats.agi[2] >= 6 && stats.luc[2] >=4;}],
	...
	PE_SILENT_DEATH: [0,1,15,29,function(n){return !traits.traits10[1] && stats.agi[2] >= 10 && (n||skills.doctor[0] >= 100) && stats.luc[2] >= 6;}],
	...
	PE_LIVING_ANATOMY: [0,1,12,29,function(n){return !traits.traits10[1] && stats.per[2] >= 7 && (n||skills.doctor[0] >= 80);}],
	...
	PE_PYROMANIAC: [0,1,9,29,function(n){return !traits.traits10[1] && stats.intl[2] > 3;}], // Мастер огня
	...
	PE_EMPATHY: [0,1,3,29,function(n){return !traits.traits10[1] && stats.cha[2] >= 6;}],
	...
	PE_ADW_MET: [0,1,1,1,function(n){return !traits.traits10[1] && !perk.PE_SILENT_RUNNING[0] /*&& stats.agi[2] >= 6*/ && (n||skills.thrown[0] >= 180)}],
	...
	PE_DEFENCE: [0,1,12,29,function(n){return stats.per[2] >= 5 && stats.enu[2] >= 8 && !traits.traits7[1] && !perk.PE_SNIPER[0]}],
...
var texttraits = {	// Трейты
	traits0: ["Быстрый метаболизм", "Ваш метаболизм превышает норму. Вам дается +60 ХП, 15 к уровню лечения. Но при этом вы сильно подвержены яду и радиации."],
	traits1: ["Крушила", "Двигаетесь помедленнее, но выглядите внушительно. Меньше на 1 ОД, но зато +3 Силы. Игнорируется эффект плазмы (тики)."],
	traits2: ["Xилое тело", "Вы не в состоянии таскать тяжелые грузы, зато более проворны. Вам дается +1 Ловкость и +5% к увороту."],
	traits3: ["Однорукий", "Вы отлично стреляете из любого одноручного оружия (+60 к навыку при расчетах, +7 к урону и игнор штрафа на силу). С двуручным уже есть проблемы (-40 к навыку при расчетах)"],
	traits4: ["Точность", "Точно, но слабо. +20% к шансу критической атаки. -10 к проверке на силу критического эффекта при атаке по вам. Но -5% к общему урону."],
	traits5: ["Камикадзе", "Вы прете напролом. +1 Ловкость, также каждые 10 секунд восполняется 1 ОД. Если не в бою - атака отнимет на 2 ОД меньше. КБ снижается до 1. Игнорируется эффект \"Подавление\"."],
	traits6: ["Громила", "Ваши жестокие удары причиняют огромные повреждения врагам (+25 к рукопашным повреждениям), но они почти никогда не бывают критическими (-30 к критроллу)."],
	traits7: ["Быстрый стрелок", "У вас нет времени для нормального прицеливания. Атаки стрелковым и метательным оружием требуют меньше на 1 ОД."],
	traits8: ["Маньяк", "По странной случайности, все вокруг вас умирают страшной и мучительной смертью. Для них вы находите худший вариант кончины. +175% Атлетизма, но -25 Очков жизней."],
	traits9: ["Дурной глаз", "Xорошая сторона дела заключается в том, что ваши враги часто ошибаются. Плохо то, что вы ошибаетесь не меньше их!"],
	traits10: ["Добродушие", "Ваши боевые навыки меньше, но зато небоевые - больше. Вы всегда думаете о защите, а не нападении (мастер-перки режут урон по вам, а не добавляют к вашему)"],
	traits11: ["Химик", "Наркотики держатся в три раза дольше. Откаты почти мгновенные. Если появится привыкание, то оно будет длиться долго."],
	traits12: ["Стабильный", "Вы критически не попадаете, но и не промахиваетесь критически. Все ваши атаки очень точны (+25 бонуса к окончательной точности)."],
	traits13: ["Жидкое тело", "Вы получаете Сопротивляемость к Урону (-10), увеличение вашего Максимального веса на 50 кг. Стимуляторы и санитар не действуют в полную мощь (-20 ХП при лечении)."],
	traits14: ["Умелец", "Параметры персонажа улучшены (+2 Выносливость, Обаяние, Интеллект, Ловкость, +25 ХП). Но вы получаете меньше способностей - одно за каждые четыре уровня опытности."],
	traits15: ["Импульсивный", "Вы очень активны, и это добавляет вам 2 ОД и 30% шанс восстановить 1 ОД при перезарядке, но вы получаете на 3 Очков умений меньше за каждый уровень. Также +20% к Метательному."]
}
var textperks = {	// Перки
