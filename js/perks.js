...
var perk = {	//(перк, уровней перка, мин уровень взятия, максимальный уровень взятия, [требования])
	...
	PE_MORE_CRITICALS: ["Больше крит. атак", "С каждым уровнем этой способности, вероятность нанесения Критических повреждений противнику увеличена на 8%.",
		2,3,29,function(n){return stats.PER[2] >= 6},
	...
	PE_SHARPSHOOTER: ["Меткость", "Искусство попадать в цель с большого расстояния. При определении дистанции вы получаете +2 к Восприятию и +6 к Дальности обзора.",
		...
		function(){pr.add("feats","oview",6);if(stats.PER[2]<3)mychar.stats.PER[1]+=2},
	...
	PE_SNIPER: ["Снайпер", "Любое попадание в цель из оружия может стать Критическим ударом. Шанс нанести Крит. урон по формуле (УД*3). -1 ОД на прицельный выстрел. +20 к Классу Брони.",
		...
		function(){pr.add("feats","crit",stats.LUC[2]*4);},
		function(){pr.add("feats","crit",-stats.LUC[2]*4);},
    ...
    PE_MENTAL_BLOCK: ["Стойкость", "Вы ну прямо очень сильны и устойчивы. Вы получаете +25 ХП, +3% к Резисту Нормы, +5% ко всем остальным Резистам. Нельзя взять Отличника или Крутизну.",
		...
		function(){for(i in resist)pr.addr(i,0,5);pr.addr("normal",0,-2);pr.add("feats","live",25);},
		function(){for(i in resist)pr.addr(i,0,-5);pr.addr("normal",0,2);pr.add("feats","live",-25);},
	...
	PE_ANIMAL_FRIEND: ["Удачный промах", "При промахе или увороте по вам, вы восстанавливаете все ОД с вероятностью (ЛВ+ОБ+ВО)% При условии не менее 70% точности противника.",
		1,15,29,function(n){return stats.AGI[2] >= 6 && stats.INT[2] >= 6},
		function(){},
		function(){},
        {stats:{AGI:6,INT:6}}],
    ...
    PE_TAG: ["Умение", "Взяв данный бонус, вы можете выбрать какое-либо Умение дополнительно. Оно будет повышаться примерно в 2 раза быстрее.",
  	...
  	PE_MUTATE: ["Мутация", "Радиация пустыни изменила вас! Вы получаете +35 Очков жизней и +100 очков распределения.",
		...
		function(){pr.add("feats","live",35);charp.points+= 100;},
		function(){pr.add("feats","live",-35);},
        {stats:{STR:3,ENU:5}}],
    ...
    PE_PRO_UDAR: ["Весельчак", "Вы не падаете при употреблении алкоголя, а наоборот получаете бонус к защите -(4+ОБ)%.",
		1,6,29,function(n){return stats.CHA[2] >= 5},
		function(){},
		function(){},
        {stats:{CHA:5}}],
	...
	PE_COMBAT_ENGINEER: ["Боевой Инженер", "Вы настолько преуспели в инженерном деле, что теперь можете применять эти навыки в бою. Вы получаете +20% к навыку Ремонт и вам доступны особые виды вооружения.",
		1,15,29,function(n){return stats.INT[2] >= 8 && stats.PER[2] >= 6 && stats.AGI[2] >= 3 && (n||skills.repair[0] >= 150 && skills.science[0] >= 120)},
		function(){pr.add("skills","repair",20,1);},
		function(){pr.add("skills","repair",-20,1);},
        {stats:{PER:6,INT:8,AGI:3},skills:{repair:150,science:120}}],
